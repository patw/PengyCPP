...
- **No console window behind the Windows GUI.** `pengy.exe` is now built as a Windows GUI app (`add_executable(pengy WIN32 …)`), so launching it from Explorer or a shortcut no longer opens a console window next to it. `--help` and `--version` attach to the terminal that launched them (unless stdout is already redirected), so they still print. Tool subprocesses need no change: `QProcess` already starts console children with `CREATE_NO_WINDOW` from a GUI parent.
- **Windows exe icon.** `pengy.exe` now embeds the Pengy icon (`pengy.ico`, 16–256px, via `pengy.rc`), so Explorer, the taskbar and shortcuts show it instead of the generic Windows program icon.

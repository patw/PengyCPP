...
void TasksDialog::loadTasks() {
    m_tasks = tasksLoad(); m_list->clear();
...
void TasksDialog::newTask() { TaskEditDialog d({}, m_theme, this); if (d.exec()==QDialog::Accepted) { taskCreate(d.title(), d.templ()); loadTasks(); } }
void TasksDialog::editTask(const QJsonObject& task) { TaskEditDialog d(task, m_theme, this); if (d.exec()==QDialog::Accepted) { taskUpdate(task["id"].toString(), d.title(), d.templ()); loadTasks(); } }
void TasksDialog::deleteTask(const QJsonObject& task) { if (QMessageBox::question(this,"Delete Task",QString("Delete task '%1'?").arg(task["title"].toString("Untitled Task")), QMessageBox::Yes|QMessageBox::Cancel, QMessageBox::Cancel)==QMessageBox::Yes) { taskDelete(task["id"].toString()); loadTasks(); } }
void TasksDialog::playTask(const QJsonObject& task) { QString templ=task["template"].toString(); QStringList ph=extractPlaceholders(templ); QMap<QString,QString> vals; if (!ph.isEmpty()) { PlaceholderDialog d(ph, m_theme, this); if (d.exec()!=QDialog::Accepted) return; vals=d.values(); } QString prompt=renderTaskTemplate(templ, vals).trimmed(); if (prompt.isEmpty()) { QMessageBox::warning(this,"Empty Task","This task produced an empty prompt."); return; } emit taskPlayed(prompt); accept(); }

...
**Windows tool surface (`run_powershell`):** Ported from Python Pengy (see its spec for the full rationale). `toolDefinitions()` is `platformTools(baseToolDefinitions(), isWindowsHost(), …)`: POSIX gets the base list unchanged; Windows gets `run_powershell` inserted before a remote-only `run_bash` (`host` required, and a local call is rejected by `windowsLocalRunBashError`), and the `download_file`/`fetch_url`/`glob` descriptions point at `run_powershell`. PowerShell is resolved once (`powershellPath()`): `pwsh` on PATH, else `powershell`, else `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe`; never `cmd.exe`. The description states the version (5.1 gets a list of syntax differences) and the elevation state (`IsUserAnAdmin` from shell32); there is no Windows elevation flow. The script is written to a `pengy-XXXXXX.ps1` `QTemporaryFile` (closed before the run, since .NET's `ReadAllText` can't share a file with an open write handle) and run as `<ps> -NoLogo -NoProfile -NonInteractive -Command <kPowershellPrelude>`; the prelude (byte-identical with the other editions, no double quotes) reads the file as UTF-8, compiles it with `[ScriptBlock]::Create`, turns off progress output and pwsh's ANSI rendering, sets UTF-8 output, exits 1 on a parse error, and otherwise exits with `$LASTEXITCODE` (1 if the script throws). From a GUI parent `QProcess` already creates console children with `CREATE_NO_WINDOW`, so no console flashes. `runPowershellLive` in `tests.cpp` drives the real prelude when `pwsh` is on PATH (it runs on Linux too).
...
On timeout or cancel, `run_bash` issues `kill -9 -PID` (killing the entire process group) before calling `QProcess::kill()`; on Windows it runs `taskkill /PID <pid> /T /F` (absolute System32 path) to kill the process tree. This prevents orphaned child processes from surviving after a timeout.
...
| `run_bash` | ❌ | `QProcess` with optional `cwd` and optional `host` (runs over ssh); sudo via `SUDO_ASKPASS` with passwords cached per host; process-group kill on timeout. On Windows remote-only: `host` is required |
| `run_powershell` | ❌ | Windows only. Run a PowerShell script locally (optional `cwd`); see "Windows tool surface" |
